...
exports.resolveProvincia = resolveProvincia;
exports.normalizeProvinciaValue = normalizeProvinciaValue;
exports.provinceOfRegione = provinceOfRegione;
exports.isSigla = isSigla;
exports.PROVINCE = [
    // Piemonte
...
const BY_SIGLA = new Map(exports.PROVINCE.map((p) => [p.sigla, p]));
const BY_NAME = new Map();
for (const p of exports.PROVINCE)
    BY_NAME.set(norm(p.nome), p);
...
function normalizeProvinciaValue(input) {
    if (!input)
        return '';
...
function provinceOfRegione(regione) {
    const target = norm(regione);
    return exports.PROVINCE.filter((p) => norm(p.regione) === target);
}
/** true se la stringa è una sigla provincia valida. */
function isSigla(input) {
